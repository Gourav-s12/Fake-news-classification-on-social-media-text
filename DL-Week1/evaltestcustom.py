...
if len(sys.argv) < 4:
    print("give file_path as 1st paramater & give model name as 2nd paramater - Dnn , Cnn, Lstm & give model_path as 3rd paramater")
    exit()
user_file_name = sys.argv[1]
user_model_name = sys.argv[2]
user_model_path = sys.argv[3]
...
test_data = pd.DataFrame({'tweet': test_df['preprocessed_tweet'], 'label': test_labels})
...
test_data = test_data
test_labels = test_data['label']
tfidf_test_vectors = tfidf_vectorizer.transform(test_data['tweet'])
...
# for DNN
if user_model_name == "Dnn":
    test_dataset = CustomDatasetDNN(tfidf_test_vectors, test_labels)
    print( tfidf_test_vectors.shape , test_labels.shape)

test_dataloader = DataLoader(test_dataset, batch_size=32)

with open(user_model_name+'_best_params.json', 'r') as f:
    best_params = json.load(f)


print(f"runing evaluation on {user_model_name}")

"""# Task 5"""

models = ["Dnn", "Cnn", "Lstm"]

if user_model_name not in models:
    print('only "Dnn", "Cnn", "Lstm" model allowed')
    exit(1)
try:
    torch.load(user_model_path, map_location=torch.device(device))
except:
    print('invalid file')
    exit(1)
...
if user_model_name == "Cnn":
    ...
    try:
        best_model.load_state_dict(torch.load(user_model_path, map_location=torch.device(device)))
    except:
        best_model2.load_state_dict(torch.load(user_model_path, map_location=torch.device(device)))
        best_model = best_model2

    get_classification_report(best_model, test_dataloader, device, model_file = "Cnn")

elif user_model_name == "Dnn":
